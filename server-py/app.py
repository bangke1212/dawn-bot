...
from database import (get_accounts, add_account, update_account, delete_account,
                      ...
                      get_logs, get_account_logs, get_stats)
...
def api_state():
    s = get_bot_state()
    s['totalAccounts'] = len(get_accounts())
    s['sessionStats'] = get_stats()
    return jsonify(s)

@app.route('/api/bot/start', methods=['POST'])
def api_start():
    iv = int(request.json.get('interval', 500)) if request.is_json else 500
    ok = start_bot(iv)
    return jsonify({'success': ok, 'message': f"Bot {'started' if ok else 'already running'} ({iv}s)"})
...
def api_run_once():
    n = run_bot_cycle()
    return jsonify({'success': True, 'message': f'Processed {n} accounts', **get_bot_state()})

@app.route('/api/accounts')
def api_accounts(): return jsonify(get_accounts())
...
def api_add():
    ...
    try: add_account(d['email'], d['token'], d.get('proxy', '')); return jsonify({'success': True})
    except Exception as e: return jsonify({'error': str(e)}), 400

@app.route('/api/accounts/<email>', methods=['DELETE'])
def api_del(email): delete_account(email); return jsonify({'success': True})
...
def api_proxies(): return jsonify(get_proxies())
...
def api_del_proxy(pid): delete_proxy(pid); return jsonify({'success': True})

@app.route('/api/logs')
def api_logs(): return jsonify(get_logs(request.args.get('limit', 100, type=int)))

@app.route('/api/logs/<email>')
def api_acct_logs(email): return jsonify(get_account_logs(email, request.args.get('limit', 50, type=int)))

@app.route('/api/stats')
def api_stats():
    s = get_stats(); accs = get_accounts()
    return jsonify({**s, 'totalAccounts': len(accs),
...
def serve(path):
    ...
    return jsonify({'message': 'DAWN Bot API (Python)', 'docs': '/api/bot/state'})
